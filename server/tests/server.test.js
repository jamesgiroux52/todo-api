...
describe('PATCH /todos/:id', () => {
    it('should update the todo by ID', (done) => {
        var hexId = todos[0]._id.toHexString();
        var text = 'The new text from update test';
        request(app)
          .patch(`/todos/${hexId}`)
...
describe('POST /users', () => {
    ...
    it('should not create a user if they already exist', (done) => {
        var email = users[0].email;
        var password = 'password52';
        request(app)
            .post('/users')
            .send({email, password})
            .expect(400)
